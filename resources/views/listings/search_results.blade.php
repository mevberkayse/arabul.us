        ...
        <div class="row justify-content-center">
            <div class="col-3 border border-secondary p-3 rounded ">
                ...
                <ul class="category-list-content">
                    ...
                    <li class="category-item mb-2"><a
                            href="/ilanlar/{{$subcategory->id}}">{{$subcategory->name}}<span>({{$subcategory->getListingCount()}})</span></a>
                    </li>
                    @endforeach
                ...
                @php
                // Keep track of parameter_ids that we've already encountered
                $seenParameterIds = [];

                // Use reject to filter out the duplicates
                $uniqueParamGroup = $paramGroup->reject(function ($param) use (&$seenParameterIds) {
                    // Check if we've already encountered this parameter_id
                    if (in_array($param->parameter_id, $seenParameterIds)) {
                        return true; // Reject this element (duplicate)
                    }

                    // Otherwise, mark this parameter_id as seen and keep it
                    $seenParameterIds[] = $param->parameter_id;
                    return false; // Do not reject this element
                });
            @endphp
                @foreach($uniqueParamGroup as $param)
                @php
                debugbar()->info($param);
            ...
            <div class="col-9">
                <div class="category-listing-title my-3">
                    <span>{{$listings->count()}} ilan bulundu</span>
                </div>
    ...
    <script>
        ...
        $('#filter').click(() => {
            ...
            $.ajax({
                ...
                method: 'POST',
                data: { params: params, _token: '{{csrf_token()}}', query: '{{request()->input("query")}}' },
                success: response => {
